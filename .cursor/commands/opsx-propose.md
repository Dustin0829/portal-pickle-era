...
When ready to implement: `/opsx-review-proposal` (auto after propose when chained) → `/opsx-apply` (auto verify → PR) → `/opsx-archive`
...
5. **Create every artifact in the required set**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      ...
      - **Inspect the relevant project before drafting**: Read `context` and `rules` first, then inspect relevant implementation, nearby tests, configuration, and documentation outside `openspec/`. Keep inspection read-only and proportional to the change; reuse findings for later artifacts and inspect more only as needed.
        ...
        - **`design.md` / `tasks.md`:** Before writing, read each **in-scope** package's `.cursor/skills/SKILL.md` (`backend`, `app`). Pick applicable rules from that index — cite paths in design/tasks; do not paste rule text.
...
- **Ladder:** `/opsx-review-proposal` (runs automatically after propose) → `/opsx-apply` (auto verify → PR) → `/opsx-archive`
- Prompt: "Review complete — read artifacts, give feedback, or run `/opsx-apply` when ready (verify + PR run automatically when tasks finish)."

**`tasks.md` requirements (when generating tasks artifact):**
- Sections: `kit.repos` ids in scope only (`api`, `web`, `plans`)
- Per section: implementation tasks citing `.cursor/rules/...` and `.cursor/skills/...` paths from that package's `SKILL.md` index
- Per section: local verify (`pnpm verify` / `openspec validate` for plans) + `merge-readiness-check` for code packages
- Final tasks: `/opsx-verify` and `/opsx-pr` checkboxes (apply auto-runs these; keep rows for progress visibility; skip verify for docs-only)
...
## Post-propose review (openspec-kit)

When `/opsx-propose` has finished creating/updating artifacts for the change
(`proposal`, `design`, `specs`, `tasks` as required by the schema):

1. Read and follow `.cursor/skills/openspec-review-proposal/SKILL.md` (`/opsx-review-proposal`) for **that same change**.
2. Strengthen artifacts for underspec bugs, edge cases, failure modes, regressions, and critical paths (planning-time — not code review).
3. Auto-apply recommended / Should-fix artifact patches; ask the user **only** for Blocking + human-judgment decisions.
4. **Loop until solid** — review → patch → full re-review until a clean pass (**zero Blocker + zero Should-fix**), or stop for human judgment / a stuck item escalated once. Nice-to-have does not keep the loop alive. Solid means the stated idea is aligned and implementable — not overengineered or scope-stretched.
5. Then **handoff** — summarize auto-updates (and autofix rounds) and wait for the user to read artifacts and either give feedback or run `/opsx-apply`.
6. Do **not** auto-run `/opsx-apply`.

Manual `/opsx-review-proposal` remains available anytime.
